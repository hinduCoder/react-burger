...
const BurgerConstructorItem = ({ item }) => {
        ...
        () => ({
            ...
            drop({ id: movedId }) {
                if (movedId !== item.localId) {
                    dispatch(moveFilling({ fromId: movedId, toId: item.localId }))
                }
            },
        }),[])
    ...
    return (
        <div className={styles.constructor_item} ref={element => drag(drop(element))} 
             style={{opacity: isDragging || isHover ? .3 : 1}}>
